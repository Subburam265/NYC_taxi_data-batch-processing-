...
def prepare_nyc_trip_data(paths: list) -> DataFrame:
    ...
    df.write.mode("overwrite").parquet("NYC_taxi_data_batch_processing/data/output_data/transformed_data")
...
def add_location_labels(
    ...
    zones_df=spark.read.csv( "NYC_taxi_data_batch_processing/data/enrichment_data/taxi_zone_lookup.csv",  header=True,  inferSchema=True).cache()
...
def add_weather_labels(df: DataFrame) -> DataFrame:
    ...
    # Fetch weather data
    weather_df = spark.read.csv("NYC_taxi_data_batch_processing/data/enrichment_data/weather_q1_2025.csv", header=True, inferSchema=True)
